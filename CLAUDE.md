...
- **Keymaps partagés entre plugins** : tous les `Alfaco*` partagent un seul espace de touches ; une collision sur une même touche se résout par ordre de chargement **alphabétique des packages** (le dernier gagne). Ex. Windows `Ctrl+Alt+M` = `init_markdown_jira` (AlfacoAtlassian) **vs** `modify_setting_from_selection` (AlfacoEditing) → ce dernier l'emporte. Voir `docs/troubleshooting.md`.
